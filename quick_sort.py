...
def _partition(arr, low, high):
    # Pilih elemen terakhir sebagai Pivot (Strategi Sederhana)
    pivot = arr[high]
    ...
    for j in range(low, high):
        ...
        HISTORY.append({
            'array': arr[:],
            'highlight': (low, high, high, 'Bandingkan'), # high: Pivot, j: Elemen yang dibandingkan
        ...
        if arr[j] <= pivot:
            ...
            # Pindahkan batas elemen yang lebih kecil
            i = i + 1
    ...
    # Lakukan penukaran pivot dengan elemen di posisi i+1
